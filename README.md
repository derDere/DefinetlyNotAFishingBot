...
3. It watches the found bobber (a lime rectangle in the preview marks where it believes the bobber is): when the matching pixels collapse into the splash or the bobber's center visibly dips down — confirmed by two consecutive scans, and never within the first seconds where no fish can bite yet — that is the bite.
